...
public class SpatialTreeMap<X,Y,V> extends AbstractMap<Coord<X,Y>,V> implements Sorted2DMap<X,Y,V>  {
  ...
  private void expandExternal(Position<Entry<Coord<X,Y>,V>> p, Entry<Coord<X,Y>,V> entry) {
    ...
    tree.addNE(p, null);         // add new sentinel leaves as children
    tree.addNW(p, null);
    tree.addSE(p, null);
    tree.addSW(p, null);
  }
  ...
  private Position<Entry<Coord<X,Y>,V>> treeSearch(Position<Entry<Coord<X,Y>,V>> p, Coord<X,Y> key) {
    ...
    int compareX = compX.compare(key.getX(), p.getElement().getKey().getX());
    int compareY = compY.compare(key.getY(), p.getElement().getKey().getY());
    if (compareX == 0) {
      if (compareY == 0) {
        return p;                        //key found; return its position
      }
      else if (compareY < 0) {
        return treeSearch(tree.se(p), key);       //search southeast subtree
      }
      else {
        return treeSearch(tree.ne(p), key);       //search northeast subtree
      }
    }
    else if (compareX < 0) {
      if (compareY >= 0) {
        return treeSearch(tree.nw(p), key);       //search northwest subtree
      }
      else { //compareY < 0
        return treeSearch(tree.sw(p), key);       //search southwest subtree
      }
    }
    else { //(compareX > 0
      if (compareY >= 0) {
