...
public class SpatialTreeMapTest {
    ...
    private SpatialTreeMap<Integer, Integer, Integer> smaller() {
    ...
    @Test
    public void testSmallerPut() {
        SpatialTreeMap<Integer, Integer, Integer> m = smaller();
        assertEquals(3, m.size());
        assertEquals(2, m.treeHeight());
    }

    @Test
    public void testSmallerGet() {
        SpatialTreeMap<Integer, Integer, Integer> m = smaller();
        assertEquals(null, m.get(new Coord<>(0,2)));
        assertEquals(null, m.get(new Coord<>(-6,-5)));
        assertEquals((int)0, (int)m.get(new Coord<>(0,0)));
        assertEquals((int)1, (int)m.get(new Coord<>(-3,4)));
        assertEquals((int)2, (int)m.get(new Coord<>(3,2)));
    }
}